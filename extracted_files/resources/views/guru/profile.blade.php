@extends('layouts.app')
@section('page_title', 'Profile Guru')
@section('styles')
<style>
    ...
    .pl { width:40%; font-weight:600; color:#555; font-size:1.15rem; }
    ...
    select.edit-input option { padding:6px; }
...
<div class="profile-card">
    ...
    <div class="profile-header">
        <div class="profile-title"><i class='bx bx-user'></i> Profile Guru</div>
        ...
        <div class="profile-body">
            <div class="profile-left">
                <table class="profile-data-table">
                    ...
                    <tr>
                        <td class="pl">NUPTK</td><td class="pc">:</td>
                        <td>
                            <span class="pv" data-field="nuptk">{{ $user->nuptk ?? '-' }}</span>
                            <input class="edit-input" type="text" name="nuptk" value="{{ $user->nuptk ?? '' }}">
                        </td>
                    </tr>
                    <tr>
                        <td class="pl">Tempat Lahir</td><td class="pc">:</td>
                        <td>
                            <span class="pv" data-field="tempat_lahir">{{ $user->tempat_lahir ?? '-' }}</span>
                            <input class="edit-input" type="text" name="tempat_lahir" value="{{ $user->tempat_lahir ?? '' }}">
                        </td>
                    </tr>
                    <tr>
                        <td class="pl">Tanggal Lahir</td><td class="pc">:</td>
                        <td>
                            <span class="pv" data-field="tanggal_lahir">{{ $user->tanggal_lahir ?? '-' }}</span>
                            <input class="edit-input" type="date" name="tanggal_lahir" value="{{ $user->tanggal_lahir ?? '' }}">
                        </td>
                    </tr>
                    <tr>
                        <td class="pl">Alamat</td><td class="pc">:</td>
                        <td>
                            <span class="pv" data-field="alamat">{{ $user->alamat ?? '-' }}</span>
                            <textarea class="edit-input" name="alamat" rows="3">{{ $user->alamat ?? '' }}</textarea>
                        </td>
                    </tr>
                    <tr>
                        <td class="pl">Mapel Yang Diajar</td><td class="pc">:</td>
                        <td style="padding-top:10px; padding-bottom:10px;">
                            <div style="display:flex; flex-wrap:wrap; gap:6px;">
                                @if(!empty($user->mata_pelajaran_ids))
                                    @foreach($user->mata_pelajaran_ids as $mId)
                                        <span style="background:#E6F1FB; color:#0C447C; padding:4px 10px; border-radius:6px; font-size:.85rem; font-weight:600; border:1px solid #cce0f5;">{{ $mapelList[$mId] ?? 'Unknown' }}</span>
                                    @endforeach
                                @else
                                    <span style="color:#888; font-size:1.05rem;">-</span>
                                @endif
                            </div>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td class="pl">Email</td><td class="pc">:</td>
                        <td>
                            <span class="pv" data-field="email">{{ $user->email ?? '-' }}</span>
                            <input class="edit-input" type="email" name="email" value="{{ $user->email ?? '' }}">
                        </td>
                    </tr>
                    <tr>
                        <td class="pl">Role</td><td class="pc">:</td>
                        <td class="pv" style="font-weight:600;color:#0C447C;">Guru</td>
                    </tr>
                </table>
...
function toggleEdit() {
    document.querySelectorAll('.pv[data-field]').forEach(s => {
        originalValues[s.dataset.field] = s.textContent.trim();
        s.style.display = 'none';
    });
    document.querySelectorAll('.edit-input').forEach(i => {
        if (!i.classList.contains('select2-hidden-accessible')) {
...
function cancelEdit() {
    ...
    const form = document.querySelector('form[action="{{ route('guru.profile.update') }}"]');
    if (form) form.reset();
}
